...
from data.dataset import Dataset, DatasetQuad
...
parser.add_argument('--resume_path', type=str, default='./output/state-ckpt-epoch-00120') # ./output/state-ckpt-epoch-00000
parser.add_argument('--loss_step', type=int, default=5)
parser.add_argument('--save_step', type=int, default=5)
parser.add_argument('--img_dims', type=list, default=[416, ]) # 608, 320
...
if __name__ == '__main__':
    ...
    datasets = [DatasetQuad(img_dim=sz) for sz in args.img_dims]
    ...
    model = DarkNet(args.model_cfg, cls_num=args.num_classes, quad=True)
    ...
    if args.resume_path:
        ops_weight_init.resume(model, optimizer=optimizer, scheduler=scheduler, state_path=args.resume_path, strict=False)
